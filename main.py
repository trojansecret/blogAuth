...
def register():
    ...
    if form.validate_on_submit():
        with app.app_context():
            ...
            if user_exists:
                flash("Email exists. Login if you have already signed up.")
            else:
                hashed_pwd = generate_password_hash(password=form.password.data, method="pbkdf2:sha256", salt_length=8)
                new_user = User(name=form.name.data, email=email, password=hashed_pwd)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        with app.app_context():
            ...
            if not user:
                flash("Email does not exist. Register if you have not signed up.")
